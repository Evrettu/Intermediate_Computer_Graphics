...
namespace nou::GLTF
{
	void LoadSkinnedMesh(const std::string& filename, SkinnedMesh& mesh, bool flipUVY)
	{
	...
	bool ExtractSkeleton(const tinygltf::Model& gltf, SkinnedMesh& mesh, JointIndexLookup& jointLookup, std::string& err, std::string& warn)
	{
	...
	bool ExtractJointLookup(const tinygltf::Model& gltf, JointIndexLookup& jointLookup, std::string& err, std::string& warn)
	{
		if (gltf.skins.size() == 0)
		{
			err = "No skeleton data in file.";
	...
	bool ExtractSkinWeights(const tinygltf::Model& gltf, SkinnedMesh& mesh, std::string& err, std::string& warn)
	{
	...
	bool ExtractSkeletalAnimation(const tinygltf::Model& gltf, SkeletalAnim& anim, const JointIndexLookup& jointLookup, std::string& err, std::string& warn)
	{
